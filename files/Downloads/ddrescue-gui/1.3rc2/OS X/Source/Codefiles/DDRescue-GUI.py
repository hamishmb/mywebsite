...
#******* There are places where UI code is duplicated (so it can be moved to look good on Mac), where a wx.Point object would work better. *** FIX ASAP.
#Import main modules
...
def GetDevInfo():
    ...
    if Linux == 1:
        ...
        subprocess.Popen(["/usr/share/ddrescue-gui/listdevices.sh", "-l"]).wait()
        ...
        # Run a short bash script to collect data about connected devices.
        subprocess.Popen(["/usr/share/ddrescue-gui/listdevices.sh", "-m"]).wait()
...
class MainWindow(wx.Frame):
    def __init__(self):
        wx.Frame.__init__(self, None, title="DDRescue-GUI v1.3rc2 (30/1/14)", size=(800,360), style=wx.DEFAULT_FRAME_STYLE ^ wx.RESIZE_BORDER)
        self.MainPanel = wx.Panel(self)

        print "DDRescue-GUI Version 1.3 rc2 UNSTABLE Starting..."
        ...
        #Recieve info from the finsihed dlg.
    ...
    def ViewOutput(self,e):
        ...
        self.ShowOutput =  subprocess.Popen(["x-terminal-emulator", "-e", "'/usr/share/ddrescue-gui/displayoutput.sh'", "--title='DDRescue-GUI: ddrescue output'",  "--hide-menubar"])
    ...
    def Reload(self,junk):
        ...
        wx.MessageDialog(self.MainPanel, 'The GUI has now been reset, and should function (and look) as it did when first started. Please check this is so, as this is an rc version.', 'DDRescue-GUI -- Warning', wx.OK | wx.ICON_EXCLAMATION).ShowModal()
...
class FinishedWindow(wx.Frame):  
    ...
    def CreateText(self):
        #Calculate length (in pixels) of string InputFileChosen, and then where on the x-axis to place it.
        ...
        if Linux == 1:
            ...
            xposForPathLabel = (350-int(width))/2
            wx.StaticText(self.FinishedPanel, -1, OutputFileChosen, pos=(xposForPathLabel,30), size=(330,15))
            wx.StaticText(self.FinishedPanel, -1, "Your recovered data is at:", pos=(85,10))
            wx.StaticText(self.FinishedPanel, -1, "Click Restart to recover more data", pos=(65,50))
            wx.StaticText(self.FinishedPanel, -1, "Click Mount to mount the image or device", pos=(40,70))
            wx.StaticText(self.FinishedPanel, -1, "Click Quit to exit the program", pos=(80,90))
        else:
            wx.StaticText(self.FinishedPanel, -1, OutputFileChosen, pos=(0,23), size=(350,15), style=WX.ALIGN_CENTER)
            wx.StaticText(self.FinishedPanel, -1, "Your recovered data is at:", pos=(85,3))
            wx.StaticText(self.FinishedPanel, -1, "Click Restart to recover more data", pos=(65,43))
            wx.StaticText(self.FinishedPanel, -1, "Click Mount to mount the image or device", pos=(40,63))
            wx.StaticText(self.FinishedPanel, -1, "Click Quit to exit the program", pos=(80,83))
...
class DevInfoWindow(wx.Frame):
    def __init__(self):
        ...
        self.okbutton = wx.Button(self.DevInfoPanel, -1, "Okay", pos=(350,270), size=(60,30))
...
class PreferencesWindow(wx.Frame):
    ...
    def CreateButtons(self):
        ...
        if Linux == 1:
            ...
            self.BestRecButton = wx.Button(self.PrefsPanel, -1, "Set to best recovery", pos=(10, 176), size=(250,30))
            self.DefaultRecButton = wx.Button(self.PrefsPanel, -1, "Balanced (default)", pos=(10, 208), size=(250,30))
            self.exitbutton = wx.Button(self.PrefsPanel, -1, "Set settings and close", pos=(10,240), size=(250,30)) 
        else:
            self.FastRecButton = wx.Button(self.PrefsPanel, -1, "Set to fastest recovery", pos=(10, 144), size=(250,30))
    ...
    def SetCMDOptions(self, event):
        ...
        if self.overwritecb.IsChecked():
            #Warn user.
            checkdlg = wx.MessageDialog(self.PrefsPanel, 'Are you sure you want to overwrite the output file? This can potentially be dangerous if you accidentally selected a device to output to. Are you sure you want to enable this option?', 'DDRescue-GUI -- Warning!', wx.YES_NO | wx.ICON_WARNING).ShowModal()
        ...
        if Blocksize == "-b Auto":
           ...
           if Linux == 1:
               getblocksize = subprocess.Popen(["/usr/share/ddrescue-gui/getblocksize.sh", "-l", "-d", InputFileChosen])
           else:
               getblocksize = subprocess.Popen(["/usr/share/ddrescue-gui/getblocksize.sh", "-m", "-d", InputFileChosen])
